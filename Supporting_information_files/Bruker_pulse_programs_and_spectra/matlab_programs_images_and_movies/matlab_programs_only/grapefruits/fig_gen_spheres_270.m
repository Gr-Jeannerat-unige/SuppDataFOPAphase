...
n=3;
pul_dur=10e-6;
angle_pulse_in_deg=90;
angle_pulse=angle_pulse_in_deg/180*pi;%deg
...
for loop_offset=0:ampli_hz/50:ampli_hz*2
    if count_main==0
        disp_on=1;
    ...
    pos_mag_p=pos_mag;
    field=[0 0 1];
    increment_tilt=pi/300000;
    inc=0;
    inc_sto=1;
    for til_tim=0:increment_tilt:n*pi/2
...
[one two]=min(sqrt(power(store_traj(:,1),2)+power(store_traj(:,2),2)))
size(store_traj)
...
cur_f.Position=[1 1 8 8];
      ...
      orient landscape
print('-depsc','-tiff','-r600',[ 'Fig_demo_spheres' num2str(n*angle_pulse_in_deg) '.eps']);%here
