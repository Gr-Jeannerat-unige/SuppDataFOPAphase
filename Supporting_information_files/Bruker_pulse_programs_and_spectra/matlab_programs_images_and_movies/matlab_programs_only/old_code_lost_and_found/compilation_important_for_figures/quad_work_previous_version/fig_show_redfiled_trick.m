...
for mainlooop=[-2:4]
    ...
    if mainlooop==1
        ...
        nuref=[nurref]*0;
        fid_ref=fid_full+exp(0*pi/2+j*2*pi*((th)*nuref));%.*exp(-th*2*pi*lb);
        fid_ref=fid_ref(1:2:end);% 1, 3, 5...
    end
    if mainlooop==0
        ti_txt='Sim. x';
        nuref=[nurref]*0;
        fid_ref=fid_full+exp(j*(pi/2+2*pi*((th)*nuref)));%.*exp(-th*2*pi*lb);
        fid_ref=fid_ref(1:2:end);% 1, 3, 5...
    end
    ...
    if mainlooop==2
        ...
        nuref=[nurref]*1;
        fid_ref=fid_full+exp(j*(1*pi/2+2*pi*((th)*nuref)));%.*exp(-th*2*pi*lb);
    end
    if mainlooop==3
    ...
    if mainlooop==-1
        ...
        nuref=-0.125;
        fid_ref=fid_full+exp(j*(000*pi/2+2*pi*((th)*nuref)));%.*exp(-th*2*pi*lb);
                fid_ref=fid_ref(1:2:end);% 1, 3, 5...
    ...
    figure(2);clf;
    plot3(t,imag(fid_sim),real(fid_sim));hold on
